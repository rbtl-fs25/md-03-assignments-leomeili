...
```{r}
sanitation_por_0020 <- sanitation %>% 
...
```{r}
ggplot(data = sanitation_por_0020,
...
1.  Look at the plot that you created. What do you notice about the order of the bars / order of the legend?
    -   it's the same order, no matter what percentages there are.
2.  What would you want to change?
    -   focus on the changes, which in my plot are difficult to decipher. as in zoom in on the ones other than 'basic sanitation services'
3.  Why did we remove "safely managed sanitation services" from the data set in Task 3?
    -   I'm not sure what they represent in this data, so I don't have an answer for this. I could guess that they might be irrelevant from a percentage point of view relative to the others?

## Task 7

1.  Run the code in the code chunk below.
2.  What do you observe when you look at the code and plot?
    -   the variable 'san_bas' is complementary to the variables 'san_lim', 'san_od' and 'san_unimp', as they all together make 100%. san_sm however is not in these 100%, so seperate from the other variables. else, there is much data to go by just for the year 2020.
